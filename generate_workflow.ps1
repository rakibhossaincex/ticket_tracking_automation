...
$START_UNIX = 1768413600

$js_code_main = @"
// ALL TEAM TICKET LOG - Enhanced with Product Type, Category-based SLA, Working Hours
...
$workflow = [PSCustomObject]@{
  nodes = @(
    ...
    [PSCustomObject]@{
      parameters  = [PSCustomObject]@{
        jsCode = "// Gate: Only process if we're past Jan 15, 2026`nconst START_UNIX = $START_UNIX;`nconst nowUnix = Math.floor(Date.now() / 1000);`n`nif (nowUnix < START_UNIX) return [];`n`nconst sd = `$getWorkflowStaticData('global');`nif (typeof sd.last_sync !== 'number') sd.last_sync = START_UNIX;`n`nreturn [{ json: { start_unix: START_UNIX, now_unix: nowUnix, last_sync: sd.last_sync } }];`n"
      }
    ...
    [PSCustomObject]@{
      parameters  = [PSCustomObject]@{
        ...
        headerParameters   = [PSCustomObject]@{
          parameters = @(
            [PSCustomObject]@{ name = "Accept"; value = "application/json" },
            [PSCustomObject]@{ name = "Intercom-Version"; value = "2.11" }
          )
        }
        options            = [PSCustomObject]@{ response = [PSCustomObject]@{} }
      }
      ...
      credentials = [PSCustomObject]@{
        httpBearerAuth = [PSCustomObject]@{ id = "NP8zyf9bSfqaxMj0"; name = "Bearer Auth account 3" }
      }
    },
    [PSCustomObject]@{
      parameters  = [PSCustomObject]@{
        jsCode = "function ensureObj(x){ if (!x) return {}; if (typeof x === 'string'){ try { return JSON.parse(x); } catch { return {}; } } return x; }`nconst resp = ensureObj(`$json);`nconst teams = resp.teams || resp.data || [];`nconst idToName = {};`nconst nameToId = {};`nfor (const t of teams) { if (!t || !t.id) continue; const name = (t.name || '').trim(); idToName[t.id] = name; if (name) nameToId[name.toLowerCase()] = t.id; }`nconst sd = `$getWorkflowStaticData('global');`nsd.teamIdToName = idToName;`nsd.teamNameToId = nameToId;`nreturn [{ json: { idToName, nameToId } }];`n"
      }
    ...
    [PSCustomObject]@{
      ...
      position = @(-2960, -2656)
      credentials = [PSCustomObject]@ {
        httpBearerAuth = [PSCustomObject]@ { id = "Wb6mK3xsTXEujIUR"; name = "Bearer Auth account 5" }
      }
    ...
    [PSCustomObject]@{
      parameters = [PSCustomObject]@{
        conditions = [PSCustomObject]@{
          boolean = @(
            [PSCustomObject]@{ value1 = "={{ `$json[\"_filter_passed\"] }}"; value2 = $true }
          )
        }
      }
    ...
    [PSCustomObject]@{
      parameters  = [PSCustomObject]@{
        ...
        headerParameters = [PSCustomObject]@{
          parameters = @(
            [PSCustomObject]@{ name = "Content-Type"; value = "application/json" },
            [PSCustomObject]@{ name = "Prefer"; value = "resolution=merge-duplicates" },
            [PSCustomObject]@{ name = "apikey"; value = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVta3pzc2Z5bXB5aGlmZGpwdHdmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2Nzk1MzkzMywiZXhwIjoyMDgzNTI5OTMzfQ.uLp84D6LmkkEL5rGgIp-EOuUX_vhNc82n-oHm6qWW-0" },
            [PSCustomObject]@{ name = "Authorization"; value = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVta3pzc2Z5bXB5aGlmZGpwdHdmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2Nzk1MzkzMywiZXhwIjoyMDgzNTI5OTMzfQ.uLp84D6LmkkEL5rGgIp-EOuUX_vhNc82n-oHm6qWW-0" }
          )
        }
        sendBody         = $true
        specifyBody      = "json"
    ...
    [PSCustomObject]@{
      parameters  = [PSCustomObject]@{
        conditions = [PSCustomObject]@{
          options    = [PSCustomObject]@{ caseSensitive = $true; leftValue = ""; typeValidation = "strict"; version = 2 }
          conditions = @(
            [PSCustomObject]@{
  ...
  connections = [PSCustomObject]@ {
    "Transform Ticket Data1" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Filter Passed?"; type = "main"; index = 0 }))) }
    "Schedule Trigger (Every 5 Minutes)" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Gate: Start on 2026-01-"; type = "main"; index = 0 }))) }
    "Gate: Start on 2026-01-" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "List Teams"; type = "main"; index = 0 }))) }
    "List Teams" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Build Team Map"; type = "main"; index = 0 }))) }
    "Build Team Map" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Build HTTP Body"; type = "main"; index = 0 }))) }
    "Build HTTP Body" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Search Tickets"; type = "main"; index = 0 }))) }
    "Search Tickets" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Paginate: Prepare next body"; type = "main"; index = 0 }, [PSCustomObject]@{ node = "Extract Tickets1"; type = "main"; index = 0 }))) }
    "Extract Tickets1" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Get Ticket Details"; type = "main"; index = 0 }))) }
    "Get Ticket Details" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Transform Ticket Data1"; type = "main"; index = 0 }))) }
    "Filter Passed?" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Upsert to Supabase"; type = "main"; index = 0 })), @(@([PSCustomObject]@{ node = "Merge"; type = "main"; index = 0 }))) }
    "Upsert to Supabase" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Merge"; type = "main"; index = 0 }))) }
    "Paginate: Prepare next body" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Merge"; type = "main"; index = 1 }))) }
    "Merge" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "If (Has Next Page?)"; type = "main"; index = 0 }))) }
    "If (Has Next Page?)" = [PSCustomObject]@{ main = @(@(@([PSCustomObject]@{ node = "Search Tickets"; type = "main"; index = 0 }))) }
  }
...
$workflow | ConvertTo-Json -Depth 10 | Out-File -FilePath "Bulk Fetch - Jan 15 to Now.json" -Encoding utf8
Write-Host "JSON is VALID and generated via PowerShell."
